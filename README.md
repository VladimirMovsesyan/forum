...
Проект собирается через docker-compose, в случае если через переменные окружения было передано DSN (Data Source Name) - проект будет конфигурироваться с Postgresql, в противном случае с in-memory-storage.